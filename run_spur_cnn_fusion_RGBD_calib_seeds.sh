#!/bin/bash
#SBATCH --job-name=spur_cnn_fusion_RGBD_calib
#SBATCH --array=1-5
...
# SPUR Variant 1 — full_spur box-family, 5 seeds
#   Model: MVStereoUNet (from-scratch CNN, in_ch=4 = RGB+depth)
#   Fusion: ON (default cross-view bottleneck fusion via 1x1 fuse_conv)
#   Input: RGB + PRO depth WITH global alpha,beta calibration (default loader).
#   Pairs with run_spur_cnn_fusion_RGBD_NOcalib_seeds.sh to isolate the
#   contribution of the PRO calibration on the spur crop set.
...
# ─────────────────────────────────────────────────────────────────────────────
# Training — MVStereoUNet (CNN) with fusion, RGB+D, alpha/beta calibration ON
# ─────────────────────────────────────────────────────────────────────────────
