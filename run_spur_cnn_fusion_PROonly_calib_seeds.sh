#!/bin/bash
#SBATCH --job-name=spur_cnn_fusion_PROonly_calib
#SBATCH --array=1-5
...
# SPUR Variant 3 — full_spur box-family, 5 seeds
#   Model:  MVStereoUNet, in_ch=1 (depth-only encoder, --no_rgb)
#   Fusion: ON  (default cross-view bottleneck fusion via 1x1 fuse_conv)
#   Pose:   ON  (default — concatenated to each view's bottleneck before fusion)
#   Input:  PRO depth WITH global alpha,beta calibration (default loader).
#   Pairs with run_spur_cnn_fusion_RGBD_calib_seeds.sh to isolate the
#   contribution of RGB on the spur crop set when PRO calibration is held on.
...
VARIANT="cnn_fusion_PROonly_calib"
...
# ─────────────────────────────────────────────────────────────────────────────
# Training — MVStereoUNet (CNN) depth-only, fusion ON, alpha/beta calibrated
# ─────────────────────────────────────────────────────────────────────────────
