#!/bin/bash
#SBATCH --job-name=spur_dino_dsb_RGBD_NOcalib_lora8
#SBATCH --array=1-5
...
# SPUR DINOv2 LoRA adapter — full_spur box-family, 5 seeds
#   Model:  MVStereoDINOUNet (DINOv2 ViT-L + DepthSideBranch + DINODecoder)
#   Backbone: FROZEN, but every attn.qkv linear is wrapped with a rank-8
#             LoRA adapter (alpha=16). Adds ~786 K trainable LoRA params on
#             top of the ~7.6 M baseline trainable head/decoder/DSB.
#   Input:   RGB + PRO depth WITHOUT global alpha,beta calibration (raw PRO)
#   Pairs with the unfrz2 run to compare "lots of unfrozen params concentrated
#   in 2 blocks" against "few low-rank params spread across all 24 blocks".
...
VARIANT="dino_dsb_RGBD_NOcalib_lora8"
...
# ─────────────────────────────────────────────────────────────────────────────
# Training — DINOv2+DSB, LoRA rank 8 on all 24 attn.qkv layers, raw PRO depth
# ─────────────────────────────────────────────────────────────────────────────
...
python MVP_MODEL/train_mvp_stereo_dino.py \
    ...
    --wandb \
    --wandb_project  "spur-encoder-ablation" \
