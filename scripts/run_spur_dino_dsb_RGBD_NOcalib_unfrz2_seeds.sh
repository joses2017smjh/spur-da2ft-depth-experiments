#!/bin/bash
#SBATCH --job-name=spur_dino_dsb_RGBD_NOcalib_unfrz2
#SBATCH --array=1-5
...
# SPUR DINOv2 partial fine-tune — full_spur box-family, 5 seeds
#   Model:  MVStereoDINOUNet (DINOv2 ViT-L + DepthSideBranch + DINODecoder)
#   Backbone: last 2 ViT-L blocks UNFROZEN, blocks 0..21 stay frozen
#             (~32.8 M trainable vs ~7.6 M baseline)
#   Input:   RGB + PRO depth WITHOUT global alpha,beta calibration (raw PRO)
#   Hypothesis: does freeing the top-of-stack ViT features lift the
#               DINOv2 stack above the from-scratch CNN on this crop set?
...
VARIANT="dino_dsb_RGBD_NOcalib_unfrz2"
