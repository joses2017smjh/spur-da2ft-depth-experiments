#!/bin/bash
#SBATCH --job-name=spur_da2_camonly_v4_10trees
#SBATCH --array=1-5
...
# v4: DA2 fine-tune on full_spur cam-only set_ids (cam1..cam9 = 9 set_ids,
# cam10 dropped to match boxfam/halfhalf sample count), 10 trees, 80/20
# tree-level split, 5 seeds. Single-image training via train_depth_da2.py
# (matches the v4 halfhalf/boxfam and the boxlr ablation protocol). Per
# (tree, set_id, shot) the manifest randomly picks _l or _r with 50/50
# probability (seeded), giving 10*9*6 = 540 samples / seed (432 train,
# 108 val with 8/2 split). No box-plate supervision (view-diversity).
...
export MANIFEST_DIR="/nfs/hpc/share/sanchej7/Computer_Vision/manifests/spur_camonly_v4_seed${SEED}"
CKPT_DIR="/nfs/hpc/share/sanchej7/Computer_Vision/checkpoints/spur_da2_camonly_v4_seed${SEED}"
...
if [ ! -f "$MANIFEST_DIR/spur_camonly_train.csv" ]; then
python3 - <<'PYEOF'
...
N_TREES  = 10
CAM_ONLY = [f"cam{i}" for i in range(1, 10)]   # 9 set_ids: cam1..cam9
BARK     = "bark_brown_02"
...
for name, ts in [("train", tr_set), ("val", va_set)]:
    ...
    with open(mdir / f"spur_camonly_{name}.csv", "w", newline="") as f:
...
python train_depth_da2.py \
    ...
    --train-manifest    "$MANIFEST_DIR/spur_camonly_train.csv" \
    --val-manifest      "$MANIFEST_DIR/spur_camonly_val.csv" \
    --save-path         "$CKPT_DIR" \
    --epochs            80 \
    --bs                2 \
    --lr                0.000005 \
    ...
    --wandb-run-name    "spur_da2_camonly_v4_seed${SEED}"
