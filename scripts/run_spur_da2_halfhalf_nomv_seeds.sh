#!/bin/bash
#SBATCH --job-name=spur_da2_halfhalf_v4_10trees
#SBATCH --array=1-5
...
# v4: DA2 fine-tune on full_spur halfhalf set_ids (canonical bark02 half-half:
# box + box_cam1..4 + cam1..4 = 9 set_ids), 10 trees, 80/20 tree-level split,
# 5 seeds. Single-image training via train_depth_da2.py (matches boxlr/boxfam/
# camonly v4). Per (tree, set_id, shot) the manifest randomly picks _l or _r
# with 50/50 probability (seeded), giving 10 * 9 * 6 = 540 samples / seed
# (432 train, 108 val with 8/2 split). No box-plate supervision (view-diversity).
...
export MANIFEST_DIR="/nfs/hpc/share/sanchej7/Computer_Vision/manifests/spur_halfhalf_v4_seed${SEED}"
CKPT_DIR="/nfs/hpc/share/sanchej7/Computer_Vision/checkpoints/spur_da2_halfhalf_v4_seed${SEED}"
...
if [ ! -f "$MANIFEST_DIR/spur_halfhalf_train.csv" ]; then
python3 - <<'PYEOF'
...
HALF_HALF = (
    ["box"]
    + [f"box_cam{i}" for i in range(1, 5)]   # box_cam1..4
    + [f"cam{i}"     for i in range(1, 5)]   # cam1..4
)   # 9 set_ids: canonical bark02 half-half
BARK  = "bark_brown_02"
...
# Fixed 10-tree pool (sorted, first 10) -> same trees across all seeds, like
# the boxlr ablation. Per-seed randomness only affects 80/20 split and l/r pick.
...
for name, ts in [("train", tr_set), ("val", va_set)]:
    ...
    with open(mdir / f"spur_halfhalf_{name}.csv", "w", newline="") as f:
...
python train_depth_da2.py \
    ...
    --wandb-run-name    "spur_da2_halfhalf_v4_seed${SEED}"
