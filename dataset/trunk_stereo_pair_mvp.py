"""
dataset/trunk_stereo_pair_mvp.py
─────────────────────────────────
Dataset for 4-view (2 stereo pairs) depth refinement using MVStereoUNet(n_views=4).
Mirrors TrunkStereoTripletMVPDataset but picks 1 nearest neighbour instead of 2.

Pairing strategy
────────────────
  • Group rows by (bark, tree, shot); requires >=2 distinct set_ids per group.
  • At init, precompute per-row nearest neighbours by camera-center distance.
  • At sample: primary + 1 nearest neighbour via inverse-distance weighting.
  • 2 rows x stereo L/R = 4 views flat: L1,R1, L2,R2.

Returned batch keys
───────────────────
  rgb    : (4, 3, H, W)    ImageNet-normalised RGB
  d_pro  : (4, 1, H, W)    input depth (PRO or Da2Finetune via INPUT_DEPTH_SUBDIR)
  d_gt   : (4, 1, H, W)    GT depth
  mask   : (4, 1, H, W)    trunk mask
  K      : (4, 3, 3)       camera intrinsics
  T_wc   : (4, 4, 4)       world-to-camera (OpenCV)
"""
...
class TrunkStereoPairMVPDataset(Dataset):
    ...
    def __init__(
        ...
        # Active index: only rows with ≥1 neighbour (need 1 for pair)
        ...
        print(f"  {Path(manifest_path).name}: {len(valid_rows):,} valid rows, "
              f"{len(self._active):,} active (≥1 neighbour) "
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # ── Draw 1 nearest neighbour with inverse-distance weighting ─────────
        ...
        n_avail = len(top_k)
        if n_avail >= 1:
            chosen = np.random.choice(n_avail, size=1, replace=False, p=weights)
        else:
            chosen = np.array([0])
        ...
        # ── Load 2 stereo pairs → 4 views flat (L1,R1, L2,R2) ─────
