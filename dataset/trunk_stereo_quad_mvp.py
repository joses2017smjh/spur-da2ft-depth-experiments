"""
dataset/trunk_stereo_quad_mvp.py
─────────────────────────────────
Dataset for 8-view (4 stereo pairs) depth refinement using MVStereoUNet(n_views=8).
Mirrors TrunkStereoTripletMVPDataset but picks 3 nearest neighbours instead of 2.

Pairing strategy
────────────────
  • Group rows by (bark, tree, shot); requires >=4 distinct set_ids per group.
  • At init, precompute per-row nearest neighbours by camera-center distance.
  • At sample: primary + 3 nearest neighbours via inverse-distance weighting.
  • 4 rows x stereo L/R = 8 views flat: L1,R1, L2,R2, L3,R3, L4,R4.

Returned batch keys
───────────────────
  rgb    : (8, 3, H, W)    ImageNet-normalised RGB
  d_pro  : (8, 1, H, W)    input depth (PRO or Da2Finetune via INPUT_DEPTH_SUBDIR)
  d_gt   : (8, 1, H, W)    GT depth
  mask   : (8, 1, H, W)    trunk mask
  K      : (8, 3, 3)       camera intrinsics
  T_wc   : (8, 4, 4)       world-to-camera (OpenCV)
"""
...
class TrunkStereoQuadMVPDataset(Dataset):
    ...
    def __init__(
        ...
        for key, group in self._scene_groups.items():
            ...
            for idx in group:
                if idx not in cam_locs or len(with_loc) < 4:
                    n_no_nb += 1
                    continue
                dists = [
        ...
        # Active index: only rows with ≥3 neighbours (need 3 for quad)
        ...
        print(f"  {Path(manifest_path).name}: {len(valid_rows):,} valid rows, "
              f"{len(self._active):,} active (≥3 neighbours) "
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # ── Draw 3 nearest neighbours with inverse-distance weighting ─────────
        ...
        n_avail = len(top_k)
        if n_avail >= 3:
            chosen = np.random.choice(n_avail, size=3, replace=False, p=weights)
        else:
            # Fallback: repeat the first available neighbour to fill
            chosen = np.array([0, min(1, n_avail - 1), min(2, n_avail - 1)])
        ...
        # ── Load 4 stereo pairs → 8 views flat (L1,R1, L2,R2, L3,R3, L4,R4) ─────
